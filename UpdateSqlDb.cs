...
namespace LAMSoft.ISACFMgr
{
	...
	{
		private SqlOperationList currentSubOperation;
		...
		internal SqlOperationList CurrentSubOperation
		{
			get
			{
				...
				if(currentSubOperation == 0)
					currentSubOperation = SqlOperationList.unknownOperation;
				return currentSubOperation;
			}
			set
			{
				currentSubOperation = value;
				LogProcessing.WriteToEventLog(string.Format("El estado de operaciones ha cambiado a: {0} -> {1}", operationList.updatingSqlDataBase.ToString(), currentSubOperation.ToString()), EventLogEntryType.Information);
		...
		internal bool UpdateSqlDataBase()
		{
			...
			ServiceSettings.serviceSettings.CurrentOperation = operationList.updatingSqlDataBase;

			ISACFDataContext db = new ISACFDataContext();
			try
			{
				if(ServiceSettings.serviceSettings.UseSqlTransaction)
				{
					Decimal AvailablePhysicalMemoryMB = Status.GetAvailablePhysicalMemoryMB();
			...
			{
				LogProcessing.WriteToEventLog(Ex.Message, System.Diagnostics.EventLogEntryType.Error);
				if(db != null)
				{
					if(db.Transaction != null)
					{
		...
		private static void UpdateDatosGen(ISACFDataContext db)
		{
			...
			FileInfo indexFile = new FileInfo(Path.Combine(ServiceSettings.serviceSettings.DataDirectory.FullName, string.Format("{0}{1}{2}", xmlDestinationFolderName, Path.DirectorySeparatorChar, indexFileName)));
		...
		private static void UpdateDomains(ISACFDataContext db)
		{
			...
			foreach(ccfCategory Category in lstCategory)
			{
				...
				GC.Collect();
				DomainsFile = new FileInfo(Path.Combine(ServiceSettings.serviceSettings.DataDirectory.FullName, string.Format("{0}{1}{2}{1}{3}", xmlDestinationFolderName, Path.DirectorySeparatorChar, Category.name, domainsFileName)));
		...
		private static void UpdateUrls(ISACFDataContext db)
		{
			...
			foreach(ccfCategory Category in lstCategory)
			{
				// Try to free some memory when Category changes.
				GC.Collect();
				UrlsFile = new FileInfo(Path.Combine(ServiceSettings.serviceSettings.DataDirectory.FullName, string.Format("{0}{1}{2}{1}{3}", xmlDestinationFolderName, Path.DirectorySeparatorChar, Category.name, urlsFileName)));
