...
namespace LAMSoft.ISACFMgr
{
	...
	public partial class ISACFMgr : ServiceBase
	{
		...
		private bool DecompressSourceFile()
		{
			...
			ServiceSettings.serviceSettings.CurrentOperation = operationList.decompressingSourceFile;

			bool result = false;

			// Form the FileInfo variable with the file to decompress.
			FileInfo fileInfo = new FileInfo(string.Format("{0}{1}", ServiceSettings.serviceSettings.DataDirectory, ServiceSettings.serviceSettings.DownloadedFileName));
			if(!fileInfo.Exists)
			{
				ServiceSettings.serviceSettings.CurrentOperation = operationList.waitingForNextExecution;
				LogProcessing.WriteToEventLog(string.Format("El estado de operaciones ha cambiado a \"{0}\", porque durante la " +
			...
			try
			{
				...
				tarGZippedArchive.ExtractContents(ServiceSettings.serviceSettings.DataDirectory.FullName);
				// Do some cleanup.
				...
				*/
				using(FileStream fsIn = new FileStream(fileInfo.FullName, FileMode.Open, FileAccess.Read))
				{
					...
					while((tarEntry = tarIn.GetNextEntry()) != null)
					{
						...
						string outName = Path.Combine(ServiceSettings.serviceSettings.DataDirectory.FullName, name);
		...
		private static void GenerateXmlFormatedIndex()
		{
			...
			ServiceSettings.serviceSettings.CurrentOperation = operationList.generatingXmlFormatedIndex;
			...
			StreamReader streamOriginalIndexContent = new StreamReader(string.Format("{0}{1}{2}{3}", ServiceSettings.serviceSettings.DataDirectory,
			...
			sbXmlResult.AppendLine(string.Format("<CategoryIndex DateCreated=\"{0}\" source=\"{1}\">", ServiceSettings.serviceSettings.DownloadedFileLastModifiedDate.ToString("o"), ServiceSettings.serviceSettings.DownloadUri));
			...
			if(!Directory.Exists(string.Format("{0}{1}", ServiceSettings.serviceSettings.DataDirectory, xmlDestinationFolderName)))
			{
				ServiceSettings.serviceSettings.DataDirectory.CreateSubdirectory(xmlDestinationFolderName);
			}

			// Full path of the xml file to create.
			string xmlIndexFilePath = string.Format("{0}{1}{2}{3}{4}", ServiceSettings.serviceSettings.DataDirectory,
			...
			string xsdGlobalUsageFileFullPath = string.Format("{0}{1}{2}", ServiceSettings.serviceSettings.DataDirectory, indexFileName, ".xsd");
		...
		{
			// Set the currentOperation
			ServiceSettings.serviceSettings.CurrentOperation = operationList.generatingXmlFormatedContent;
			...
			string xsdFileFullPath = string.Format("{0}{1}", ServiceSettings.serviceSettings.DataDirectory, "domains.xsd");
			GenerateXsdDomainsFile(xsdFileFullPath);
			// Add the schema to the collection.
			schemaSet.Add(null, xsdFileFullPath);

			// Generate urls.xsd
			xsdFileFullPath = string.Format("{0}{1}", ServiceSettings.serviceSettings.DataDirectory, "urls.xsd");
			...
			foreach(XAttribute name in CategoryNames)
			{
				...
				string workingDir = string.Format("{0}{1}{2}{3}{4}", ServiceSettings.serviceSettings.DataDirectory,
																											sourceFolderName, Path.DirectorySeparatorChar,
																											CategoryFolderName, Path.DirectorySeparatorChar);
				if(Directory.Exists(workingDir))
				{
					string destXmlFolderFullPath = string.Format("{0}{1}{2}{3}{4}", ServiceSettings.serviceSettings.DataDirectory, xmlDestinationFolderName,
		...
		private static void XmlValidationCallBack(object sender, System.Xml.Schema.ValidationEventArgs args)
		{
			if(args.Severity == System.Xml.Schema.XmlSeverityType.Warning)
				LogProcessing.WriteToEventLog(string.Format("Advertencia: discordancia en el esquema mientras se " +
				...
				LogProcessing.WriteToEventLog(string.Format("Error al comprobar el esquema (schema) XML del fichero de indice " +
